...
function genererDocxComposeur(dossierSource, variantesChoisies, idTheme, formatPage, sansAccroche) {
  ...
  // TACHE (chantier Mini CV A5 -- refonte Portrait) : dossierSource
  // (metierCible) et recommandationsIACV (competencesAValoriser) transmis
  // en plus -- necessaires a composeurComposerA5Portrait pour reutiliser
  // unifierEtPlafonnerCompetences() (js/app.js) telle quelle, jamais une
  // 2e logique de fusion/tri de competences ecrite a la main.
  var composition = composeurComposer(objetCV, profil, decisions, variantesChoisies || {}, formatPage, theme, dossierSource, recommandationsIACV);
...
// TACHE (bouton "Mettre en avant + regrouper", point 14 cv.md -- chantier
// "experiencesRetenues", remplace l'ancien objet unique experiencePrioritaire) :
// remplace objetCV.experiences par [UNE experience detaillee par entree de
// experiencesRetenues (ordre de priorite de l'IA respecte, pro et perso
// melangees -- chacune affichee comme une experience normale du bloc
...
// groupe restant (poste = metiers joints par une virgule, missions =
// texteRegroupe en une seule ligne, jamais decoupee)]. Copie defensive de
// objetCV (jamais modifie sur place), meme principe que le reste de ce
// moteur. Repli silencieux sur objetCV inchange si rien d'exploitable n'a
// ete propose par l'IA (aucune experience retenue valide ET aucun groupe)
// -- n'est appelee que si theme.regroupementActif est vrai, mais reste
// prudente meme dans ce cas (l'IA peut ne rien avoir propose pour ce profil).
function composeurAppliquerRegroupementExperiences(objetCV, regroupementExperiences) {
  var reg = regroupementExperiences || {};
  var retenues = reg.experiencesRetenues || [];
  ...
  // equivalents) : verifie que CHAQUE experience retenue correspond
  ...
  // faite pour illustrePar (app.js). Une entree hallucinee est simplement
  // ignoree (jamais toute la fonctionnalite bloquee pour une seule entree
  // invalide -- contrairement a l'ancien comportement objet unique, ou une
  // seule entree invalide annulait tout le regroupement).
  var experiencesReelles = objetCV.experiences || [];
  var experiencesPersoReelles = objetCV.experiencesPersonnelles || [];
  var retenuesValides = retenues.filter(function (r) {
    return (r.type === 'professionnelle' && r.poste && experiencesReelles.some(function (e) {
      return (e.poste && correspond(e.poste, r.poste)) || (e.entreprise && r.entreprise && correspond(e.entreprise, r.entreprise));
    })) ||
    (r.type === 'personnelle' && r.intitule && experiencesPersoReelles.some(function (e) {
      return e.intitule && correspond(e.intitule, r.intitule);
    }));
  });

  if (retenuesValides.length === 0 && groupes.length === 0) { return objetCV; }

  // Ordre de la liste = ordre de priorite de l'IA, jamais reordonne ici.
  var itemsRetenus = retenuesValides.map(function (r) {
    return {
      poste: r.type === 'professionnelle' ? r.poste : r.intitule,
      entreprise: r.type === 'professionnelle' ? (r.entreprise || '') : '',
  ...
  // (missions completes pour chaque experience retenue, 1 ligne pour
  // chaque groupe) : aucune troncature supplementaire n'est necessaire ni
  // souhaitable.
