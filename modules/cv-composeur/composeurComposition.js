...
var COMPOSEUR_CAPACITES_A4_DETAILLE_CV = {
  ...
  // de "capacites" (Essentiel/Intégral) laisse .slice(0, undefined) tout garder.
  logiciels: 8,
...
// TACHE (chantier Mini CV A5 -- refonte Paysage) : l'ancienne fonction de
// composition dediee au Paysage (contenu FIXE, jamais mobile) a ete
// retiree -- Paysage reutilise desormais EXACTEMENT la meme composition
// que Portrait (composeurComposerA5Portrait ci-dessous, contenu mobile +
// competences fusionnees triees par pertinence), seul l'AGENCEMENT visuel
// change (colonne centrale dediee au bloc identite, voir composeurRender.js)
// -- jamais une 2e logique de composition dupliquee pour le meme contenu.
// TACHE (chantier Mini CV A5 -- refonte Portrait, plan valide avec
// l'utilisateur) : capacites de depart dediees, distinctes de
// CAPACITES_A5_CV (formatA5CV.js, pipeline legacy miniCvA5.js -- jamais
// touchee ici) et de COMPOSEUR_CAPACITES_A4_DETAILLE_CV (page A4 entiere,
// bien plus de place). Chiffres proposes par Claude, a confirmer par
// l'utilisateur une fois vus en vrai (voir memoire projet) : 2
// experiences (2 missions max chacune), 1-2 formations, ~6 competences
// fusionnees, 2-3 loisirs, 2 langues. Engagements/certifications/savoir-
// faire personnel : secondaires, 1 seul chacun, affiches uniquement s'il
// y a du contenu ET si la place le permet (jamais annonces explicitement
// par l'utilisateur dans sa liste de priorite).
var CAPACITES_A5_PORTRAIT_CV = {
  experiences: 2, missionsParExperience: 2, formations: 2, competences: 6,
  loisirs: 3, langues: 2, engagements: 1, certifications: 1, competencesPersonnelles: 1,
  // TACHE (rubrique « Logiciels et outils » dédiée, décision Denis 2026-08-28) :
  // affichée aussi sur le Mini CV A5 (« tous les formats »), mais très
  // resserrée -- demi-page A4, place comptée.
  logiciels: 4
};

// TACHE (chantier Mini CV A5 -- refonte Portrait) : contenu MOBILE (pas
// fixe comme l'ancien code, voir composeurComposerA5PaysageLegacy
// ci-dessus) -- bloc identite (metier/nom/coordonnees/permis) prepare a
// part (jamais dans les colonnes, voir composeurRender.js), puis 5
// rubriques + 3 secondaires repartie librement entre 2 colonnes par un
// budget de hauteur ESTIME (nombre de lignes), dans l'ordre de priorite
// donne par l'utilisateur : Experiences -> Competences -> Formations ->
// Centres d'interet -> Langues -> (secondaires). Contrainte capacite
// stricte : ne jamais depasser une demi-page A4 -- cette premiere passe
// utilise un budget estime a la composition (avant tout rendu reel) ;
// un rééquilibrage sur la HAUTEUR REELLEMENT RENDUE (meme esprit que
// mesurerHauteursColonnesXXL/essayerRequilibrageFormationsAutomatique,
// js/app.js) reste un chantier de suite, pas encore branche ici.
// TACHE (retour utilisateur : "compétences fusionnées, pas séparées,
// triées par pertinence au métier visé") : reutilise
// unifierEtPlafonnerCompetences() (js/app.js, deja existante -- fusionne
// savoirFaire+savoirEtre+savoirs en 1 seul pool, trie par recommandations
// IA puis referentiel metier puis reste du dossier, plafonne) -- jamais
// une 2e logique de fusion/tri ecrite a la main pour ce format.
function composeurComposerA5Portrait(objetCV, theme, dossierSource, recommandationsIACV) {
  var optionDebordementA5 = (theme && theme.optionDebordement) || null;
  // TACHE (retour utilisateur : "débordement -- les 2 options doivent
  // s'appliquer normalement à l'A5") : option B (comme pour le Paysage
  // legacy) réduit ici le NOMBRE d'éléments retenus par rubrique.
  var echelleB = (optionDebordementA5 === 'B' || optionDebordementA5 === 'AB') ? 0.75 : 1;
  function plafond(cle) { return Math.max(1, Math.round(CAPACITES_A5_PORTRAIT_CV[cle] * echelleB)); }

  var recoCompetences = ((recommandationsIACV && recommandationsIACV.competencesAValoriser) || []).map(function (c) {
    return { texte: c.competence, justification: c.justification };
  });
  var metierCible = (dossierSource && dossierSource.metierCible) || '';
  var poolCompetences = (typeof unifierEtPlafonnerCompetences === 'function')
    ? unifierEtPlafonnerCompetences(objetCV.competences, recoCompetences, metierCible, plafond('competences'))
    : { savoirFaire: ([].concat((objetCV.competences && objetCV.competences.savoirFaire) || [], (objetCV.competences && objetCV.competences.savoirEtre) || [])).slice(0, plafond('competences')) };
  var competencesFinales = poolCompetences.savoirFaire || [];

  var experiencesA5 = (objetCV.experiences || []).slice(0, plafond('experiences')).map(function (e) {
    var copie = {};
    Object.keys(e).forEach(function (k) { copie[k] = e[k]; });
    if (copie.missions) {
      var lignesMissions = String(copie.missions).split('\n').filter(Boolean);
      copie.missions = lignesMissions.slice(0, plafond('missionsParExperience')).join('\n');
    }
    return copie;
  });
  ...
  // a l'A4 (cvPdfDonnees.js, meme fusion), objetCV.experiencesPersonnelles
  // (experience personnelle/benevolat, Projet XXL) n'etait jamais lu ici --
  // une personne l'ayant saisie la voyait disparaitre silencieusement sur
  ...
  function hauteurRubrique(cle) {
    switch (cle) {
      case 'experiences': return experiencesA5.length ? 1 + experiencesA5.reduce(function (s, e) { return s + 1 + nbLignesMissions(e); }, 0) : 0;
      case 'competences': return competencesFinales.length ? 1 + competencesFinales.length : 0;
      case 'formations': return formationsA5.length ? 1 + formationsA5.length : 0;
      case 'loisirs': return loisirsA5.length ? 1 + loisirsA5.length : 0;
      case 'langues': return languesA5.length ? 1 + languesA5.length : 0;
      case 'logiciels': return logicielsA5.length ? 1 + logicielsA5.length : 0;
      case 'engagements': return engagementsA5.length ? 1 + engagementsA5.length : 0;
      case 'certifications': return certificationsA5.length ? 1 + certificationsA5.length : 0;
      case 'competencesPersonnelles': return competencesPersoA5.length ? 1 + competencesPersoA5.length : 0;
      default: return 0;
    }
  }

  // TACHE (meme esprit que essayerRequilibrageFormationsAutomatique/
  // essayerRequilibrageLateralAutomatique, js/app.js) : glouton par ordre
  // de priorite, chaque rubrique rejoint la colonne actuellement la plus
  // courte -- jamais un emplacement fixe impose par rubrique.
  var ordrePriorite = ['experiences', 'competences', 'logiciels', 'formations', 'loisirs', 'langues', 'engagements', 'certifications', 'competencesPersonnelles'];
  var hauteurGauche = 0, hauteurDroite = 0;
  var colonneGauche = [], colonneDroite = [];
  ordrePriorite.forEach(function (cle) {
    var h = hauteurRubrique(cle);
    if (h <= 0) { return; }
    if (hauteurGauche <= hauteurDroite) { colonneGauche.push(cle); hauteurGauche += h; }
    else { colonneDroite.push(cle); hauteurDroite += h; }
  });

  // TACHE (retour utilisateur : "le rajustement automatique doit
  // fonctionner pour l'A5, Portrait comme Paysage") : la repartition
  // ci-dessus n'est qu'une ESTIMATION (budget de lignes), avant tout rendu
  // reel -- corrige ici par un deplacement d'appoint decide apres mesure
  // REELLE du rendu (essayerRequilibrageA5Automatique/activerMiseEnFormeUltimeXXL,
  // js/app.js, meme mecanisme que blocDroiteDeplaceGauche/blocLateralDeplaceDroite
  // pour l'A4 -- jamais une 2e logique de deplacement ecrite a la main).
  if (theme && theme.blocA5DeplaceGauche && colonneDroite.indexOf(theme.blocA5DeplaceGauche) !== -1) {
    colonneDroite = colonneDroite.filter(function (r) { return r !== theme.blocA5DeplaceGauche; });
    colonneGauche.push(theme.blocA5DeplaceGauche);
  }
  if (theme && theme.blocA5DeplaceDroite && colonneGauche.indexOf(theme.blocA5DeplaceDroite) !== -1) {
    colonneGauche = colonneGauche.filter(function (r) { return r !== theme.blocA5DeplaceDroite; });
    colonneDroite.push(theme.blocA5DeplaceDroite);
  }
  ...
  return {
    ...
    loisirs: loisirsA5,
    langues: languesA5,
    logiciels: logicielsA5,
    engagements: engagementsA5,
...
function composeurComposer(objetCV, profil, decisions, variantesChoisies, formatPage, theme, dossierSource, recommandationsIACV) {
  variantesChoisies = variantesChoisies || {};
  // TACHE (format Mini CV A5 -- Portrait, refonte) : branchement précoce,
  // avant toute la logique de stratégie/capacités/débordement A4 -- ce
  // format a ses propres capacites et sa propre repartition en colonnes
  // (composeurComposerA5Portrait ci-dessus), jamais celles d'A4.
  // TACHE (format Mini CV A5 -- Paysage, refonte) : reutilise desormais la
  // MEME composition que Portrait (contenu mobile, competences fusionnees
  // triees par pertinence) -- seul l'agencement (colonne centrale dediee
  // au bloc identite au lieu d'un bandeau) differe cote rendu.
  if (formatPage === 'A5-portrait' || formatPage === 'A5-paysage') {
    var compositionA5 = composeurComposerA5Portrait(objetCV, theme, dossierSource, recommandationsIACV);
  ...
  // TACHE (rubrique « Logiciels et outils » dédiée, décision Denis 2026-08-28) :
  // 'logiciels' ajouté aux rubriques supportées ET au repli sans stratégie --
  // sans ça, le filtre ci-dessous le retirerait de l'ordre décidé par R005.
  var RUBRIQUES_SUPPORTEES = ['profil', 'experiences', 'formations', 'competences', 'logiciels', 'competencesCles', 'langues', 'loisirsEngagements', 'competencesPersonnelles'];
  var ordreRubriquesPrincipales = strategieCV
    ...
    : ['profil', 'experiences', 'formations', 'competences', 'logiciels', 'langues', 'loisirsEngagements'];
  ...
  // TACHE (retour utilisateur, revirement assume : "en 'Par competences'
  // je n'ai plus du tout le bloc Experience personnelle, meme avec de la
  // place disponible -- je dois les avoir") : "Centre d'interet" (loisirs)
  // et "Experience personnelle" (engagements + experiencesPersonnelles)
  // etaient jusqu'ici couples via une variable unique
  // (loisirsEngagementsPresent = 'loisirsEngagements' present dans
  // ordreRubriques de la strategie) -- la strategie "Par competences"
  // (STRATEGIES_CV, composeurStrategies.js) ne liste pas
  // 'loisirsEngagements' (pensee avant l'existence du bloc Projet XXL
  // "Experience personnelle", jamais connu de la strategie de contenu),
  // ce qui supprimait a tort les 2 blocs a la fois pour cette strategie.
  // Decision explicitement inversee ici : "Experience personnelle" ne
  // depend plus QUE du contenu reel (voir plus bas), plus de cette
  // variable -- supprimee, devenue inutile. "Centre d'interet" continue
  // de dependre naturellement de la presence de 'loisirsEngagements' dans
  // ordreRubriquesPrincipales/laterale (filtre plus haut), sans variable
  // separee necessaire.
  ...
  //
  // TACHE (Projet XXL, "CV Intégral") : quasiment aucun plafonnement --
  // 2 pages sont explicitement autorisées, "aucune réduction A/B ne s'y
  // applique" (doc Projet XXL). Une clé ABSENTE de "capacites" fait que
  // tout .slice(0, capacites.xxx) reçoit "undefined" comme borne, ce qui
  // renvoie le tableau COMPLET (comportement JS volontaire, pas un
  // oubli) -- vrai pour experiences/formations/langues/loisirs/
  // certifications/engagements, gardés intégralement.
  // TACHE (retour utilisateur : "compétences professionnelles et
  // comportementales, on s'arrête à 10 chacune -- le but c'est de
  // remplir un maximum, mais pas les compétences au-delà de ça") : SEULE
  // exception à "aucun plafonnement" -- competences/competencesPersonnelles
  // ont ici leur propre plafond FIXE, distinct de
  // COMPOSEUR_CAPACITES_A4_DETAILLE_CV (5/5) et de CAPACITES_A4_ESSENTIEL_CV
  // (6, fusionné) -- jamais relié au mécanisme de bonus (js/app.js,
  // essayerToutesRubriquesBonusSiPossible() l'exclut explicitement pour
  // ce format), une limite ferme, pas un point de départ à dépasser.
  // TACHE (retour utilisateur : "il y a une limite des compétences par
  ...
  var capacites = estFormatIntegral
    ? { competences: 10, competencesPersonnelles: 10 }
    : formatEssentiel
  ...
  // non affiché avant de toucher au style -- savoir-être, loisirs,
  // certifications, formations, langues, engagements") : même clonage
  // defensif que capaciteExperiencesBonus, étendu à theme.bonusCapacites
  // (objet { rubrique: bonus }, voir composeurTheme.js) -- un seul bonus
  // par rubrique suffit à justifier le clonage, jamais recalculé sinon.
  var unBonusCapaciteActif = theme.bonusCapacites && Object.keys(theme.bonusCapacites).some(function (k) { return theme.bonusCapacites[k]; });
  if (estProjetXXL && (theme.capaciteExperiencesBonus || unBonusCapaciteActif)) {
    var capacitesEtendues = {};
    Object.keys(capacites).forEach(function (k) { capacitesEtendues[k] = capacites[k]; });
    capacitesEtendues.experiences = (capacitesEtendues.experiences || 0) + (theme.capaciteExperiencesBonus || 0);
    if (theme.bonusCapacites) {
      Object.keys(theme.bonusCapacites).forEach(function (cle) {
        if (theme.bonusCapacites[cle]) { capacitesEtendues[cle] = (capacitesEtendues[cle] || 0) + theme.bonusCapacites[cle]; }
      });
    }
  ...
  if (estProjetXXL && (optionDebordement === 'B' || optionDebordement === 'AB') && !estFormatIntegral) {
    // TACHE (retour utilisateur : "je clique sur Retirer des missions, je
    // compare 2 exports, absolument rien ne change" -- bug réel confirmé
    // en comparant 2 .docx réels au caractère près) : cause trouvée -- dès
    // que 3 expériences professionnelles ou plus existent, le plafond
    // appliqué PLUS HAUT (lignesMaxParExperience, ligne ~458) réduit déjà
    // chaque expérience à 1 seule ligne de mission AVANT que ce bloc ne
    // s'exécute. "Retirer 1 ligne sur 3" n'a alors plus rien à retirer
    // (lignes.length < 2, garde ci-dessous) -- un CV avec 4 expériences
    // (le cas exact signalé) retombe donc TOUJOURS sur ce no-op, quel que
    // soit le contenu réel. Ne remplace pas le retrait "1 sur 3" (toujours
    // tenté en premier, inchangé si suffisant) -- ajoute un 2e palier,
    // déclenché UNIQUEMENT quand le premier n'a rien pu retirer : retire
    // alors la ligne de mission restante EN ENTIER (garde le titre/dates,
    // seule la ligne de détail disparaît) -- même garde "jamais l'expérience
    // la plus mise en avant" (index === 0) que ci-dessus.
    experiencesRetenues = experiencesRetenues.map(function (e, index) {
      ...
      if (lignes.length < 2) {
        if (lignes.length === 0) { return e; } // deja vide, rien a faire
        var copieVidee = {};
        Object.keys(e).forEach(function (k) { copieVidee[k] = e[k]; });
        copieVidee.missions = '';
        return copieVidee;
      }
  ...
  var contenuRetenu = {
    ...
    // TACHE (rubrique « Logiciels et outils » dédiée, décision Denis 2026-08-28) :
    // dossier.logiciels (tableau de chaînes) est déjà normalisé dans
    // objetCV.logiciels (normaliserDonneesCV.js) -- simple passe-plat plafonné
    // ici, jamais une relecture de dossier en aval. Rubrique jamais affichée si
    // la liste est vide (garde-fou dans composeurRender.js, comme certifications).
    logiciels: (objetCV.logiciels || []).slice(0, capacites.logiciels),
    loisirs: (objetCV.loisirs || []).slice(0, capacites.loisirs),
    // TACHE (missions de loisir, retour utilisateur : "un centre d'intérêt
    // mis en avant... il faut aussi lui donner des missions") : simple
    // passage direct -- deja calcule par js/app.js (appliquerMoteurDecisionCV),
    // jamais recalcule ici. null si aucun loisir retenu (repli silencieux
    // sur l'affichage habituel, voir composeurRender.js).
    loisirRetenu: objetCV.loisirRetenu || null,
    ...
    // puisse s'étendre s'il reste de la place, comme les autres rubriques") :
    // réutilise TEL QUEL le bonus "competencesPersonnelles" (theme.bonusCapacites,
    // voir essayerToutesRubriquesBonusSiPossible(), js/app.js) -- jamais
  ...
  // masquer) : point de centralisation unique. theme.rubriquesMasquees vient
  // de composeurAppliquerReglagesProjetXXL (reglagesProjetXXL.rubriques). On
  // vide la liste concernee : "vide = jamais affichee" est deja le contrat
  // (langues via bandeau, certifications, logiciels...). Le rendu et la
  // repartition en colonnes (plus bas) sont deja gardes par des tests
  // `.length`, rien d'autre a toucher. Le permis (en-tete) est gere par
  // theme.permisMasque, propage dans composition.permisMasque.
  var _rubMasq = (theme && theme.rubriquesMasquees) || {};
  if (_rubMasq.langues) { contenuRetenu.langues = []; }
  if (_rubMasq.certifications) { contenuRetenu.certifications = []; }
  if (_rubMasq.logiciels) { contenuRetenu.logiciels = []; }
  if (_rubMasq.loisirs) { contenuRetenu.loisirs = []; contenuRetenu.loisirRetenu = null; }
  if (_rubMasq.engagements) { contenuRetenu.engagements = []; }
  if (_rubMasq.experiencesPersonnelles) { contenuRetenu.experiencesPersonnelles = []; }
  ...
  // TACHE (rubrique « Logiciels et outils » dédiée, décision Denis 2026-08-28) :
  // 'logiciels' = liste compacte de mots -> colonne latérale, juste après
  // 'competences' et avant 'langues' (position validée avec Denis).
  var RUBRIQUES_PREFEREES_LATERALE = ['competences', 'logiciels', 'langues', 'loisirsEngagements', 'competencesPersonnelles'];
  ...
  if (estProjetXXL) {
    var ORDRE_LATERALE_XXL = ['competences', 'logiciels', 'langues', 'competencesPersonnelles', 'loisirsEngagements'];
  ...
  // remonter en tete.
  // TACHE (bug corrige, voir commentaire plus haut) : depend desormais
  // UNIQUEMENT du contenu reel, jamais de la presence d'un autre slot
  // ('loisirsEngagements') dans une strategie qui l'ignore.
  if (estProjetXXL &&
  ...
  if (estProjetXXL && nombreColonnes === 1) {
    ...
    // TACHE (rubrique « Logiciels et outils » dédiée, décision Denis 2026-08-28) :
    // en 1 colonne, juste après le bloc compétences combiné, avant les
    // expériences -- rubrique à part entière (jamais fondue dans les compétences).
    if (contenuRetenu.logiciels && contenuRetenu.logiciels.length) { ordre1Colonne.push('logiciels'); }
  ...
  // TACHE (retour utilisateur : "l'engagement/le savoir-faire non retenu
  // se retrouve collé après les missions détaillées de l'autre, sans
  // rapport visible") : sépare l'élément CHOISI par l'IA
  // (`retenuMiseEnAvant`, posé par js/app.js) des éléments NON retenus --
  // désormais commun aux 2 modes de colonnes (extrait ici, avant la
  // branche 2 colonnes qui suit plus bas, pour ne jamais dupliquer cette
  // logique). En 2 colonnes, les non-retenus partent à gauche (voir plus
  // bas, repartitionColonnes.laterale) ; en 1 colonne (pas de colonne
  // latérale où les envoyer), ils restent dans le même bloc mais
  // composeurRender.js leur donne chacun un petit sous-titre par type
  // ("Engagement"/"Savoir-faire personnel") pour ne pas les confondre
  // avec l'élément développé -- voir sa propre documentation. Uniquement
  // si le bloc "Expérience personnelle" est bien présent -- sinon rien à
  // séparer, repli sur le comportement fusionné existant, inchangé
  // (dossiers sans cette recommandation IA, ou aucun élément retenu).
  if (estProjetXXL && repartitionColonnes.principale.indexOf('experiencesPersonnelles') !== -1) {
    var expPersoPourSplit = contenuRetenu.experiencesPersonnelles || [];
    var engagementsPourSplit = contenuRetenu.engagements || [];
    var estRetenu = function (e) { return !!(e && typeof e === 'object' && e.retenuMiseEnAvant); };
    var uneRetenueExiste = expPersoPourSplit.some(estRetenu) || engagementsPourSplit.some(estRetenu);
    if (uneRetenueExiste) {
      var expPersoNonRetenues = expPersoPourSplit.filter(function (e) { return !estRetenu(e); });
      var engagementsNonRetenus = engagementsPourSplit.filter(function (e) { return !estRetenu(e); });
      contenuRetenu.experiencesPersonnelles = expPersoPourSplit.filter(estRetenu);
      contenuRetenu.engagements = engagementsPourSplit.filter(estRetenu);
      contenuRetenu.savoirFairePersonnelNonRetenu = expPersoNonRetenues;
      contenuRetenu.engagementNonRetenu = engagementsNonRetenus;
      // TACHE : uniquement en 2 colonnes -- ces 2 rubriques ne sont
      // poussées en colonne latérale que là où une colonne latérale
      // existe reellement (voir plus bas). En 1 colonne, ces memes
      // listes restent lues directement par composeurRender.js pour un
      // rendu INLINE (sous-titres), jamais en rubrique separee.
      if (nombreColonnes === 2) {
        if (expPersoNonRetenues.length) { repartitionColonnes.laterale.push('savoirFairePersonnelNonRetenu'); }
        if (engagementsNonRetenus.length) { repartitionColonnes.laterale.push('engagementNonRetenu'); }
      }
    }
  }
  ...
  if (estProjetXXL && nombreColonnes === 2) {
    ...
    // équilibré") : déplacement RÉEL entre colonnes -- contrairement à
    // blocMisEnAvantGauche/Droite plus bas (qui ne font que réordonner
    // À L'INTÉRIEUR d'une colonne déjà fixée, jamais entre les 2, voir
    // leur .indexOf() qui exige la présence PRÉALABLE dans cette même
    // colonne). Fait AVANT ce passage-là exprès, pour que
    // blocMisEnAvantDroite==='formations' (si jamais actif en même
    // temps) retombe silencieusement en no-op plutôt que de planter --
    // le déplacement automatique l'emporte, cas limite jugé acceptable
    // (theme.formationsDeplaceesGauche n'est de toute façon jamais
    // activé par une vraie personne, seulement par le bouton "1 page,
    // lisible"). dessinerRubrique() (composeurRender.js) accepte déjà
    // "colonne" en paramètre pour CHAQUE rubrique, "formations" y
    // compris -- aucun nouveau rendu à écrire, seul le déplacement dans
    // repartitionColonnes suffit.
    // TACHE (retour utilisateur : "formation, certification, engagements
    // -- les petites rubriques doivent pouvoir passer a gauche, pas
    // seulement Formations") : generalise -- theme.blocDroiteDeplaceGauche
    // porte desormais le NOM de la rubrique a deplacer (candidat unique a
    // la fois, choisi par essayerRequilibrageFormationsAutomatique/
    // essayerRequilibrageColonnesSiPossible parmi CANDIDATS_DROITE_VERS_GAUCHE_XXL,
    // js/app.js), jamais seulement 'formations' en dur.
    if (theme.blocDroiteDeplaceGauche && repartitionColonnes.principale.indexOf(theme.blocDroiteDeplaceGauche) !== -1) {
      repartitionColonnes.principale = repartitionColonnes.principale.filter(function (r) { return r !== theme.blocDroiteDeplaceGauche; });
      repartitionColonnes.laterale.push(theme.blocDroiteDeplaceGauche);
    }
    // TACHE (retour utilisateur : "la colonne de gauche est trop dense,
    // ça laisse un trou considérable à droite") : mécanisme INVERSE du
    // précédent -- déplacement réel d'un bloc de compétences depuis la
    // colonne latérale vers la principale (jamais 'loisirsEngagements',
    // qui reste toujours à gauche, voir plus haut). Même position dans
    // le fichier, pour la même raison (retombe en no-op silencieux si
    // blocMisEnAvantGauche visait ce même bloc, jamais un plantage).
    if (theme.blocLateralDeplaceDroite && repartitionColonnes.laterale.indexOf(theme.blocLateralDeplaceDroite) !== -1) {
      repartitionColonnes.laterale = repartitionColonnes.laterale.filter(function (r) { return r !== theme.blocLateralDeplaceDroite; });
      repartitionColonnes.principale.push(theme.blocLateralDeplaceDroite);
    }
    // TACHE (retour utilisateur : "si les compétences comportementales
    // passent à droite, je veux compétences professionnelles en 1er,
    // suivi des formations et diplômes à condition qu'elles soient
    // développées (avec missions), suivies par Engagement et Centre
    // d'intérêt en dernier") : les deux ajustements ci-dessus ajoutent
    // 'formations' (via .push, donc toujours en dernière position) ou
    // retirent une rubrique de la latérale -- sans jamais reconsidérer
    // l'ORDRE final. Réordonne ici une dernière fois selon une séquence
    // fixe, un simple filtre sur ce qui est réellement présent (jamais
    // une liste écrite en dur qui ferait apparaître une rubrique absente),
    // pour que 'formations' ne se retrouve jamais après 'loisirsEngagements'
    // (Centre d'intérêt) juste parce qu'il a été ajouté après coup.
    // 'experiencesPersonnelles' (Engagement) est TOUJOURS en colonne
    // principale à ce jour (voir plus bas) -- inclus ici par prudence,
    // sans effet tant qu'aucun mécanisme ne le fait passer en latérale.
    if (estProjetXXL) {
      var formationDeveloppee = (theme.blocMisEnAvant === 'formations') ||
        (theme.blocMisEnAvantGauche === 'formations') || (theme.blocMisEnAvantDroite === 'formations');
      var ORDRE_LATERALE_FINAL_XXL = formationDeveloppee
        ? ['competences', 'logiciels', 'langues', 'formations', 'competencesPersonnelles', 'experiencesPersonnelles', 'loisirsEngagements']
        : ['competences', 'logiciels', 'langues', 'competencesPersonnelles', 'experiencesPersonnelles', 'formations', 'loisirsEngagements'];
      repartitionColonnes.laterale = ORDRE_LATERALE_FINAL_XXL.filter(function (r) { return repartitionColonnes.laterale.indexOf(r) !== -1; });
    }
    ...
    // TACHE (retour utilisateur : "si je mets en avant les compétences
    // comportementales, le texte déplacé à droite par le réequilibrage
    // automatique doit revenir à gauche tout en haut -- et ce qui occupe
    // la gauche à sa place [Formations] doit repartir à droite") : la
    // mise en avant EXPLICITE de la personne l'emporte toujours sur le
    // réequilibrage automatique (blocLateralDeplaceDroite, juste
    // au-dessus) -- jamais l'inverse. Fusionne les 2 cas (bloc déjà à
    // gauche : simple réordonnancement ; bloc renvoyé à droite par
    // l'automatisme : rapatriement + échange avec Formations) en un seul
    // mécanisme, plutôt que 2 vérifications séparées comme avant --
    // 'loisirsEngagements' n'est de toute façon jamais déplacé vers la
    // droite (exclu de CANDIDATS_LATERAL_VERS_DROITE_XXL, js/app.js),
    // donc pour lui ce n'est jamais qu'un simple réordonnancement.
    if (theme.blocMisEnAvantGauche && blocGaucheAContenuReel(theme.blocMisEnAvantGauche)) {
      var blocGaucheVoulu = theme.blocMisEnAvantGauche;
      var blocGaucheEtaitADroite = repartitionColonnes.principale.indexOf(blocGaucheVoulu) !== -1;
      if (blocGaucheEtaitADroite || repartitionColonnes.laterale.indexOf(blocGaucheVoulu) !== -1) {
        repartitionColonnes.principale = repartitionColonnes.principale.filter(function (r) { return r !== blocGaucheVoulu; });
        repartitionColonnes.laterale = repartitionColonnes.laterale.filter(function (r) { return r !== blocGaucheVoulu; });
        repartitionColonnes.laterale.unshift(blocGaucheVoulu);
        // Libère de la place à gauche : "Formations" (seul contenu que le
        // mécanisme symétrique formationsDeplaceesGauche peut avoir fait
        // migrer vers la gauche) repart à droite, jamais gardé à côté du
        // bloc mis en avant.
        if (blocGaucheEtaitADroite && repartitionColonnes.laterale.indexOf('formations') !== -1) {
          repartitionColonnes.laterale = repartitionColonnes.laterale.filter(function (r) { return r !== 'formations'; });
          repartitionColonnes.principale.push('formations');
        }
      }
    }
  ...
  // R007 (echelle) + R003 (plancher dur, jamais contourne).
  ...
  // n'y a de sens).
  // TACHE (retour utilisateur : "je clique sur Réduire la police, je
  // compare 2 exports, absolument rien ne change" -- bug réel confirmé en
  // comparant 2 .docx réels au caractère près) : le calcul ci-dessous
  // (Math.max(decisions.tailleMinPolice.valeur, Math.round(11 * echelle)))
  // clampe DÉJÀ 11 * 1 (echelle neutre, cas courant -- aucun bonus densité
  // "faible") à 12pt (R003) AVANT toute réduction -- 11 * 0.83 ≈ 9pt
  // retombe ENCORE sur ce même plancher de 12pt une fois clampé : les deux
  // calculs donnent exactement 12pt, aucune différence visible, quel que
  // soit le contenu. R003 reste un plancher dur pour toute génération
  // AUTOMATIQUE (jamais touché ici) -- mais un clic EXPLICITE sur ce
  // bouton doit avoir un effet réel (accord explicite de l'utilisateur) :
  // plancher distinct, plus bas, réservé à ce seul cas.
  var reductionPoliceExplicite = estProjetXXL && (optionDebordement === 'A' || optionDebordement === 'AB') && !estFormatIntegral;
  if (reductionPoliceExplicite) { echelle = echelle * 0.83; }
  ...
  // utilisé par R007 pour un contenu "faible" en 1 page) tant qu'aucun
  ...
  // R007 (composeurRegles.js) applique déjà un bonus GÉNÉRIQUE de +15%
  ...
  // générique de R007, ni pour Essentiel/Intégral qui ont leurs propres
  // règles) -- calibré empiriquement (voir tests) à +15% supplémentaire
  // (compose avec R007 : 1.15 × 1.15 ≈ 1.32 au total), au-delà duquel le
  ...
  // TACHE (retour utilisateur, suite du plancher R003 ci-dessus) : 10.5pt
  // choisi avec l'utilisateur (fourchette 10.5-11pt proposée, validée) --
  // reste raisonnablement lisible (à peine sous le standard 11-12pt), tout
  // en garantissant un vrai effet visuel au clic. Ne s'applique QUE sur
  // demande explicite (reductionPoliceExplicite) -- toute génération
  // automatique/par défaut garde R003 (12pt) intact, inchangé.
  var PLANCHER_POLICE_REDUCTION_EXPLICITE = 10.5;
  var plancherPoliceActif = reductionPoliceExplicite ? PLANCHER_POLICE_REDUCTION_EXPLICITE : decisions.tailleMinPolice.valeur;
  var taillePoliceCorps = Math.max(plancherPoliceActif, Math.round(11 * echelle));
  ...
  // puces, entre les blocs), DECOUPLE de l'echelle de police.
  // TACHE (retour utilisateur, port PDF puis capture d'ecran reelle a
  // l'appui : "espace considérable entre Compétences comportementales et
  // Expérience professionnelle... je veux que le texte soit davantage
  // agrandi plutôt que d'avoir des espaces comme ça") : 1.8 (valeur
  // d'origine) produisait un espacement disproportionne une fois visible
  // sur un vrai rendu -- ramene a 1.3, un gain plus modeste mais moins
  // "vide qui saute aux yeux" entre les rubriques. Le levier "police"
  // n'est PAS augmente en compensation : deja teste au-dela (voir plus
  // haut, echelle 1.3 -> ~16pt jugee "disproportionnee pour un CV
  // professionnel") -- mieux vaut une page pas totalement remplie qu'une
  // police trop grande OU des espaces trop larges.
  // Base : remplissage AUTO d'une page clairsemee (densite de CONTENU faible).
  // x le choix DENSITE de la personne (theme.densiteEspacementUtilisateur,
  // 1 = normal, ~1.22 = aere, ~0.82 = compact -- chantier "La mise en page",
  // lot moteur sous-lot 1). Les 2 leviers se composent : un CV leger regle
  // sur "compact" reste plus aere qu'un CV plein regle sur "compact".
  // x le reglage FIN "Espacement des paragraphes" (theme.espacementParasMult,
  // lot moteur sous-lot 2) : se compose avec les 2 leviers ci-dessus.
  var espacementExtra = ((estDetaille && profil.densite === 'faible') ? 1.3 : 1)
    * ((theme && theme.densiteEspacementUtilisateur) || 1)
    * ((theme && theme.espacementParasMult) || 1);
  ...
  return {
    ...
    // (gauche / justifie), lu par composeurRender (Word) et cvPdfTemplateA4
    // (PDF). Vient de theme.alignementCorps (composeurTheme.js).
    alignementCorps: (theme && theme.alignementCorps) || 'gauche',
    // Lot moteur sous-lot 6 : un "non" explicite de la personne
    // (theme.veuvesUtilisateur === false) desactive le controle ; sinon
    // l'auto (decisions) decide, comme avant.
    controleVeuvesOrphelines: (theme && theme.veuvesUtilisateur === false)
      ? false
      : decisions.controleVeuvesOrphelines.valeur,
    // Lot moteur sous-lot 2 : reglages fins interligne + marges de page.
    // interligneCorps (ratio, 1 = neutre) -> line spacing des paragraphes de
    // corps Word + line-height PDF. margesTwips -> marges de section Word (A4
    // Projet XXL). margeLateralePdf -> padding lateral des conteneurs PDF.
    interligneCorps: (theme && theme.interligneCorps) || 1,
    margesTwips: (theme && theme.margesTwips) || 560,
    margeLateralePdf: (theme && theme.margeLateralePdf) || '14mm',
    // Lot moteur sous-lot 4 : le permis se masque via l'en-tete (les 2
    // moteurs lisent objetCV.permis directement, jamais contenuRetenu).
    permisMasque: !!(theme && theme.permisMasque),
    // TACHE (retour utilisateur : "A4 Essentiel identique à A4 Détaillé") :
