/* ============================================================
   ...
   Point d'entrée SÉCURISÉ entre la réponse de l'assistant et le reste
   du module : aucun autre fichier ne doit relire ou réinterpréter le
   JSON brut de l’assistant — tous reçoivent uniquement la structure déjà
...
// orthographiée, inventée par l’assistant) retombe sur DECOUVERTE_ORIGINES.AUTRE,
...
// réponse de l’assistant comme une autre, elle doit passer par EXACTEMENT la même
...
// Valide une compétence proposée par l’assistant pour un fragment. Retourne
...
function _decouverteValiderFragment(fragmentBrut, indexOriginal) {
  ...
  return {
    ...
    // "fragments" tel que renvoyé par l’assistant -- c'est cet indice que
    ...
    // l’assistant, transporté ensuite par initialiserEtatFragment()
...
// TACHE (retour Denis, 2026-08-31, Chantier 4) : validation minimale des
// questionsCiblees dans le cas "saisie inexploitable" -- il n'y a pas de
// fragment, donc fragmentIndex est toujours null et type toujours "texte".
// On garde juste le texte des questions (pistes de reecriture du recit).
function _decouverteQuestionsSansFragments(brutes) {
  if (!Array.isArray(brutes)) { return []; }
  return brutes
    .map(function (q) {
      if (typeof q === 'string') {
        return q.trim() ? { texte: q.trim(), fragmentIndex: null, type: 'texte' } : null;
      }
      if (!q || typeof q !== 'object') { return null; }
      var t = (typeof q.texte === 'string') ? q.texte.trim() : '';
      return t ? { texte: t, fragmentIndex: null, type: 'texte' } : null;
    })
    .filter(Boolean)
    .slice(0, 10);
}
...
function analyserReponseDecouverte(texteColle) {
  if (!(texteColle || '').trim()) {
    return { succes: false, erreur: 'Aucun texte à analyser. Collez la réponse de l\'assistant avant de continuer.' };
  }
  ...
  // Deux formes possibles cote assistant : soit { "analyseImpossible": true,
  // "message": "..." } (sortie de secours du prompt), soit un JSON avec
  // "fragments": [] (l'assistant n'a rien trouve). Les deux -> MEME signal :
  // ce n'est ni un souci de copier-coller, ni un echec de format ; le texte
  // de depart ne permet rien. L'appelant (decouverteParcours.js) affiche
  // alors un ecran DEDIE ("reprenez votre recit"), jamais la fenetre de
  // validation des fragments, jamais un message d'erreur qui blame le collage.
  // On remonte les questionsCiblees : elles servent de pistes de reecriture.
  if (objetBrut.analyseImpossible === true || !fragments.length) {
    return {
      succes: false,
      analyseImpossible: true,
      erreur: 'Saisie inexploitable : aucune activité identifiée dans le texte.',
      message: (typeof objetBrut.message === 'string' && objetBrut.message.trim())
        ? objetBrut.message.trim() : null,
      questionsCiblees: _decouverteQuestionsSansFragments(objetBrut.questionsCiblees)
    };
  }
  ...
  var questionsCiblees = questionsBrutes
    .map(function (q) {
      ...
      // en contexte non structuré vers l’assistant. Repli sur "texte" pour toute
  ...
  // CV") : ne jamais dépendre du seul jugement de l’assistant pour ça -- vérifié
  // ici, en code, systématiquement.
  // TACHE (retour utilisateur : "4 métiers identifiés, je m'attendais à 4
  // questions de date -- je n'en ai eu qu'une partie") : bug réel trouvé
  // -- l'ancien filet de sécurité ne couvrait que le cas "AUCUNE question
  // de date du tout" (un seul fragment couvert, le premier fragment pro),
  // jamais le cas où l’assistant a bien pensé à UNE date mais en a oublié
  // d'autres parmi plusieurs métiers -- alors que la consigne
  // (decouverte-competences.md : "plusieurs métiers distincts identifiés :
  // demande TOUJOURS, pour CHACUN dont la date n'est pas connue, une
  // question séparée") en demande une par fragment professionnel sans
  // date, pas une seule au total. Parcourt désormais TOUS les fragments
  // professionnels (proDeclaree/proNonDeclaree, même définition que
  // l'ancien "premierFragmentPro"), ajoute une question de repli pour
  // CHACUN qui n'a pas déjà une question de type "date" pointant vers lui
  // (fragmentIndex) -- jamais pour un fragment déjà couvert par l’assistant,
  // jamais deux fois pour le même.
  // TACHE (chantier "exp perso") : la détection repose sur q.type ===
  // 'date' (fiable, l’assistant l'indique explicitement) -- l'ancien repérage
  // par mots-clés dans le texte de la question n'est plus nécessaire (il
  // restait fragile, voir historique des bugs déjà rencontrés sur cette
  // détection).
  var fragmentsProSansDate = fragments.filter(function (f) {
    if (f.origine !== 'proDeclaree' && f.origine !== 'proNonDeclaree') { return false; }
    return !questionsCiblees.some(function (q) { return q.type === 'date' && q.fragmentIndex === f.indexOriginal; });
  });
  if (fragmentsProSansDate.length) {
    var questionsDateRepli = fragmentsProSansDate.map(function (f) {
      // TACHE : extrait de texteBrut plutôt qu'une question générique
      // ("cette activité") -- indispensable dès qu'il y a PLUSIEURS
      // fragments sans date en même temps, sinon la personne ne saurait
      // pas laquelle des questions identiques concerne quelle expérience
      // (même principe que decouverte-competences.md :99, "jamais une
      // question générale qui suppose une seule période de vie").
      var extrait = f.texteBrut.length > 60 ? f.texteBrut.slice(0, 60) + '…' : f.texteBrut;
      return {
        texte: 'Pendant quelle(s) période(s) (années, même approximatives) avez-vous exercé cette activité : « ' + extrait + ' » ?',
        fragmentIndex: f.indexOriginal,
        type: 'date'
      };
    });
    // En priorité, avant les questions déjà proposées par l’assistant -- quitte
    // à retirer ses dernières questions pour respecter le plafond de 10
    // (même choix qu'avant : les dates manquantes priment).
    questionsCiblees = questionsDateRepli.concat(questionsCiblees).slice(0, 10);
  }
  ...
  // responsabilité de l’assistant (decouverte-competences.md), jamais reconstruit
...
// Export CommonJS protege -- tests/decouverteLogique.test.js (Node),
// aucun effet sur le chargement navigateur classique (balise <script>,
// ou `module` n'est jamais defini). Depend des globales
// DECOUVERTE_ORIGINES/DECOUVERTE_CATEGORIES (decouverteClassification.js)
// et extraireBlocJSONDepuisTexte (js/app.js), a fournir par l'appelant.
if (typeof module !== 'undefined') {
  module.exports = {
    analyserReponseDecouverte: analyserReponseDecouverte,
    decouverteValiderCategorie: decouverteValiderCategorie,
    decouverteValiderCompetenceProposee: decouverteValiderCompetenceProposee
  };
}
